...
namespace Dc.CrawlWikiMed
{
    class Program
    {
        static void Main()
        {
            ...
            var saveDataPath = Utility.GetAbsolutePath(typeof(Program).Assembly.Location, "../../../SysptomTraining.txt");
            File.WriteAllLines(saveDataPath, diseasesNameAndSymptom.Select(q => $"{q.Value}|{q.Key}"), Encoding.UTF8);
